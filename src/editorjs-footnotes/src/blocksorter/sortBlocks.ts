...
async function getFootnoteBlockIndex(
  footnoteID: string,
  editor: EditorJS,
): Promise<number> {
...
async function popFootnoteBlocks(
  editor: EditorJS,
  ...
  while (i < editor.blocks.getBlocksCount()) {
    ...
    if (
      blockObject["data"] &&
      blockObject["tool"] == "footnoteParagraph" &&
      blockObject["data"]["text"]
    ) {
...
async function addFoonotesBack(
  footnotes: { [id: string]: { data: any } },
  editor: EditorJS,
) {
